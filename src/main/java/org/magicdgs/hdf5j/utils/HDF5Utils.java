...
public final class HDF5Utils {
     ...
     * the {@link HDF5Constants#HDF5_FORMAT_SIGNATURE}.
     *
    ...
    public static boolean isHDF5Stream(final InputStream inputStream) throws IOException {
        ...
        inputStream.mark(HDF5Constants.HDF5_FORMAT_SIGNATURE.length);
     ...
     * {@link HDF5Constants#HDF5_FORMAT_SIGNATURE}.
     *
    ...
    private static boolean readHDF5signature(final InputStream inputStream) throws IOException {
        ...
        for (int i = 0; i < HDF5Constants.HDF5_FORMAT_SIGNATURE.length; i++) {
            final int currentByte = inputStream.read();
            if (currentByte == -1 || currentByte != HDF5Constants.HDF5_FORMAT_SIGNATURE[i]) {
                return false;
            }
        }
        return true;
